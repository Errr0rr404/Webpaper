...
final class DesktopWindow: NSWindow {
	...
	convenience init(display: Display?) {
		...
		self.collectionBehavior = [
			.stationary,
			.ignoresCycle,
			.fullScreenNone // This ensures that if Webpaper is launched while an app is fullscreen (fullscreen is a separate space), it will not show behind that app and instead show in the primary space.
		]
