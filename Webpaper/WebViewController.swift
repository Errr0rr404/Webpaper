...
final class WebViewController: NSViewController {
	/// Keeps popup windows alive until they close. A single stored window used to leak every popup except the latest.
	private let popupWindows = PopupWindowKeeper()
	/// Failures are values, not a publisher completion. Completing on the first error made every later load invisible to the app.
	private let didLoadSubject = PassthroughSubject<Result<Void, Error>, Never>()
	...
	*/
	...
	private func createWebView() -> SSWebView {
		...
		userContentController.addJavaScript("document.documentElement.classList.add('is-webpaper-app')")
	...
	private func internalOnLoaded(_ error: Error?) {
		...
		if let error, !WKWebView.canIgnoreError(error) {
			didLoadSubject.send(.failure(error))
			return
		}

		didLoadSubject.send(.success(()))
	}
}
...
extension WebViewController: WKNavigationDelegate {
	func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction) async -> WKNavigationActionPolicy {
		...
		{
			// Hide Webpaper if it's in front of everything.
...
/// Retains browsing-mode popup windows until AppKit finishes closing them.
private final class PopupWindowKeeper: NSObject, NSWindowDelegate {
	...
	func windowWillClose(_ notification: Notification) {
		...
		// Drop the reference on the next turn. Releasing the window inside `windowWillClose` can deallocate it while AppKit is still closing it.
