...
final class WebsitesController {
	...
	*/
	func remove(_ website: Website) {
		...
		let removedID = website.id.uuidString
		let assignments = Defaults[.displayWebsites].filter { $0.value != removedID }
		if assignments.count != Defaults[.displayWebsites].count {
			Defaults[.displayWebsites] = assignments
		}
	}
