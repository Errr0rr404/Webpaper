...
final class AppState: ObservableObject {
	...
	*/
	...
	/// Load failures keyed by web view controller. A success on one display must not hide a failure on another, and a later success must clear a stale error.
	private var loadErrors: [ObjectIdentifier: Error] = [:]
...
//		webViewController.reloadCurrentPageFromOrigin()
	...
	*/
	...
	Called exactly once, when the instance is created. The subscription lives on the instance and is released when the instance is torn down. Load failures are values on the publisher, not a terminal completion, so a later load can still clear the error.
	*/
	private func wireEvents(for instance: DesktopInstance) {
		...
		webViewController.didLoadPublisher
			.sink { [self] result in
				let key = ObjectIdentifier(webViewController)
	...
	/**
	The website rendered by `webView`, including a per-display assignment. Falls back to the menu-bar selection when the web view is not one of the desktop instances.
	*/
	func website(for webView: WKWebView) -> Website? {
		guard let instance = desktopInstances.first(where: { $0.webViewController.webView === webView }) else {
			return WebsitesController.shared.current
		}

		return WebsitesController.shared.website(forDisplay: instance.targetDisplay)
	}

	private func publishLoadError() {
		webViewError = loadErrors.values.first
	}

	private func pruneLoadErrors() {
		let live = Set(desktopInstances.map { ObjectIdentifier($0.webViewController) })
		guard loadErrors.contains(where: { !live.contains($0.key) }) else {
			return
		}

		loadErrors = loadErrors.filter { live.contains($0.key) }
		publishLoadError()
	}
}
