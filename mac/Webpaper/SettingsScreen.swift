...
struct SettingsScreen: View {
	var body: some View {
		...
		.frame(width: 400)
		.fixedSize()
		.windowLevel(.floating + 1) // To ensure it's always above the Webpaper browser window.
	}
}
...
private struct ShowOnAllSpacesSetting: View {
	var body: some View {
		...
		.help("While disabled, Webpaper will display the website on the space that is active at launch.")
	}
}
...
private struct OpenExternalLinksInBrowserSetting: View {
	var body: some View {
		...
		.help("If a website requires login, you should disable this setting while logging in as the website might require you to navigate to a different page, and you don't want that to open in a browser instead of Webpaper.")
	}
}
...
private struct HideMenuBarIconSetting: View {
	...
	var body: some View {
		Defaults.Toggle("Hide menu bar icon", key: .hideMenuBarIcon)
			...
			.alert2(
				"If you need to access the Webpaper menu, launch the app again to reveal the menu bar icon for 5 seconds.",
